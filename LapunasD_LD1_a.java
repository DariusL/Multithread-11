...
public class LapunasD_LD1_a {
    ...
    public void otherMain(){
    	final ArrayList<ArrayList<Struct>> duomenys = skaitymas("LapunasD.txt");
    	final int procesuSkaicius = duomenys.size();

        List<Thread> procesai = new ArrayList<>(procesuSkaicius);
        System.out.print("Sinchroninis spausdinimas\n");
        antraste();
        for(int i = 0; i < procesuSkaicius; i++)
        	spausdinti(duomenys.get(i), "Procesas_" + i + " ");
        System.out.print("\n\n\n");
        for(int i = 0; i < procesuSkaicius; i++){
            procesai.add(new Thread(new SuperRunnable(i) {
                ...
                @Override
                public void run() {
                    spausdinti(duomenys.get(getNr()), "Procesas_" + getNr() + " ");
                }
            }));
        }
    ...
    public static void antraste(){
    	System.out.printf("%10s %2s %15s %7s %10s\n", "Procesas", "Nr", Struct.PAV, Struct.KIEKIS, Struct.KAINA);
    }
    
    public static void main(String[] args) {
        LapunasD_LD1_a app = new LapunasD_LD1_a();
        app.otherMain();
    }
}